    ...
    <section>
        <div style="text-align:center;">
         <div style="margin:1.0rem;">
            <img :src="alipayMoneyQrcode" alt="暂无上传" style="width:8.0rem;height:8.0rem;"/>
        </div>
        <div style="margin:20px;">
           <van-uploader accept="image/jpeg,image/png,image.jpg" multiple :after-read="onRead">
          <van-button size="large" type="default" style="width:8.0rem;background:red;color:#ffffff">上传支付宝收钱二维码</van-button>
          </van-uploader>
        </div>
...
export default {
  ...
  methods: {
      ...
      if (_this.id == "") {
        ...
        this.$axios
          ...
          .then(function(response) {
            ...
            _this.alipayMoneyQrcode =
              _this.userdata.alipay + "?time=" + times;
            if (_this.alipayMoneyQrcode == "?time=" + times) {
              _this.alipayMoneyQrcode = _this.scan;
            }
          })
    ...
    onRead(file) {
      ...
      this.alipayMoneyQrcode = windowURL.createObjectURL(this.filepath);
      let formdatas = new FormData();
      formdatas.append("alipay", this.filepath);
      ...
      this.$axios
        .post(
          _this.url + "/user/bind-alipay?userId=" + _this.id,
          formdatas,
          config
        )
        .then(function(response) {
